<template>
  <div>
    <div id="cesiumContainer" ref="cesiumContainer" />
		    <!-- 界线信息面板 -->
    <BoundaryInfomationPage :boundaryinfo="boundaryDataForm" />
  </div>
</template>
...
import BoundaryInfomationPage from './components/boundary-information-page'

export default {
  name: "boundaryMap",
  components: {
    DialogDrag,
    BoundaryInfomationPage
  },
  data() {
    return {
			viewer: null,
		...
		loadData(){
			this.viewer.dataSources.add(
				Cesium.GeoJsonDataSource.load(
					"/geojson/boundary/市级界线.json",
				)
			);
			this.viewer.dataSources.add(
				Cesium.GeoJsonDataSource.load(
					"/geojson/boundary/县级界线.json",
				)
			);
			this.viewer.dataSources.add(
				Cesium.GeoJsonDataSource.load(
					"/geojson/boundary/乡级界线.json",
				)
			);
			this.viewer.dataSources.add(
				Cesium.GeoJsonDataSource.load(
					"/geojson/boundary/村级管理线.json",
				)
			);
      // // 设置3D模型mouse事件交互
      // interactOperate.install(this.viewer, this.boundaryDataForm)
        ...
        handlerCli.setInputAction(function (movement) {
            ...
            if (Cesium.defined(pick)) {
               const Bid = pick.primitive._instanceIds;
							// const Bid = pick.primitive
							 console.log(Bid);
            }
